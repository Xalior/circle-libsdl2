...
| Display/renderer queries (modes, bounds, formats, masks) | single HDMI panel. The mode list is the standard sizes that fit inside it, and setting one resizes the virtual display - see [The mode list](DISPLAY.md#the-mode-list) |
| Keyboard → SDL events, `SDL_GetKeyboardState`, modifiers, lock states and lamps | Circle USB HID (raw reports; SDL scancodes *are* USB usage codes). Scancodes are physical positions and do not move with the keyboard layout. Caps lock, num lock and scroll lock are reported as states in the modifier word, from the keyboard layout that holds them, and the keyboard's own lamps are lit to match by a transfer that is submitted and not waited for (not on the Pi 3, whose USB controller has no asynchronous control transfer). Num lock gives the keypad its navigation meaning. Off core 0: USB stays on core 0, events cross by ring |
| Mouse → SDL events, `SDL_GetMouseState`, relative mode, warping | Circle USB mouse (raw reports). A mouse says how far it moved and never where it is, so the library keeps the position itself and clamps it to the window. The cursor is drawn by this library, composed last over the frame, in canvas coordinates - `SDL_CreateCursor`, `SDL_CreateColorCursor`, `SDL_CreateSystemCursor` and `SDL_SetCursor` are real, and there is a default arrow. Off core 0: USB and event synthesis stay on core 0; the position and buttons are held in memory both cores see |
