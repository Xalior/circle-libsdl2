...
| Keyboard → SDL events, `SDL_GetKeyboardState`, modifiers, lock states | Circle USB HID (raw reports; SDL scancodes *are* USB usage codes). Scancodes are physical positions and do not move with the keyboard layout. Caps lock, num lock and scroll lock are reported as states in the modifier word, from the keyboard layout that holds them. Off core 0: USB stays on core 0, events cross by ring |
