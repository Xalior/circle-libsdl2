...
- **There is one framebuffer grant on the board, and everything that draws shares it.** Asking the firmware for it is what sets the display mode, so two askers with two different requests would be two modes. A host kernel that puts its log on the screen ([LOGGING.md](LOGGING.md)) makes that grant during its own initialisation, with the same request an application's first window would have made; the window then adopts it. The screen stops being a log destination the moment an application initialises SDL video, so the console and the application never hold the framebuffer at the same time.
