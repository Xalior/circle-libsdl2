...
- **A blended draw into a target leaves the pixels it touched opaque.** The blitter composites the source colour over the destination and writes a fully opaque result, because the frame it was written for goes to a panel that has no transparency. So a target used as a translucent overlay keeps transparency only where nothing was blended over it: a clear with an alpha of zero stays transparent, and an unblended copy carries its source's alpha through unchanged, but the pixels a `SDL_BLENDMODE_BLEND` copy touched come out opaque. Desktop SDL composites the destination alpha too, and this is the one place a target here answers differently.
