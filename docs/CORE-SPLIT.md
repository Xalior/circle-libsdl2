...
**The application core has one job**: the program's own main loop, and as little else as possible. Presentation and scaling go to the chosen presentation core; devices, files and the sound feed go to core 0. The program itself is the only work that stays on the application core, because it is the only work that cannot be moved.

**The topology is chosen, not assumed, and the same compiled binary runs every role on a single core as well.** A board with cores to spare and a board with none run the same image; only the host kernel's choice differs. The single-core case is the degenerate path - no ring, no mailbox, every call direct.

Every crossing in this library is a mailbox or a ring that any core may consume, and nothing in it is written against a particular core number: the library does not decide where anything runs.
