...
3. **Call `SDL2Circle_ArmCoreRuntime` as the first statement on every core**, core 0 included. A core that has just started has no thread pointer, and C++ exception state is reached through it, so the first thrown exception dereferences whatever the firmware left in that register. Where the leftover value is zero the read goes to mapped low memory and the throw appears to work — which is why omitting this call works on one board and takes a data abort on the next, on an ordinary throw, looking exactly like a hardware fault. It is one call and it is not optional. On core 0 it also starts the C++ threading runtime, which is why that needs nothing of its own; see [C++ threading](THREADING.md).
