# posix.mk — build the missing POSIX functions into a Circle kernel.
#
# Include this on the line before Circle's Rules.mk and there is nothing
# else to do. It finds itself, so the directory can be anywhere:
#
#   include /wherever/circle-posix/posix.mk
#   include $(CIRCLEHOME)/Rules.mk
#
# It adds its object to OBJS, its directory to INCLUDE, and the rules to
# build both the object and — where the project wants one — its dependency
# file. Its directory on the include path is also how a consumer reaches the
# three headers the C library does not have at all: <sys/utsname.h>,
# <sys/termios.h> and <dlfcn.h>.
#
# The one thing this fragment does not settle is the library's own include
# path. posix.cpp calls the I/O service, so the project must already reach
# SDL2/SDL_circle.h — which any project linking this library does.
#
...
#
# The compile rule is written out rather than left to Rules.mk's pattern
# rule because a pattern rule only matches sources beside the makefile that
...
POSIX_DIR := $(patsubst %/,%,$(dir $(lastword $(MAKEFILE_LIST))))
...
POSIX_OBJDIR ?= $(OBJDIR)
ifeq ($(strip $(POSIX_OBJDIR)),)
POSIX_OBJDIR := .
endif

POSIX_OBJ := $(POSIX_OBJDIR)/posix.o
...
$(POSIX_OBJ:.o=.d): $(POSIX_DIR)/posix.cpp
	@mkdir -p $(dir $@)
	@$(CPP) $(CPPFLAGS) -M -MG -MT $(POSIX_OBJ) -MT $@ -MF $@ $<
endif
